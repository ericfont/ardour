	...
	if (nominally_recording || (re && _was_recording && _session.get_record_enabled() && punch_in)) {
		...
		if (rec_nframes && !_was_recording) {
			...
			if (loop_loc) {
				...
				*/
				_capture_captured     = start_sample - loop_start.samples();;
			...
			if (_midi_write_source) {
				...
				start = timepos_t (_capture_start_sample);
	...
	if (rec_nframes) {
		...
		if (_midi_buf) {
			...
			for (MidiBuffer::iterator i = buf.begin(); i != buf.end(); ++i) {
				...
				*/
				const samplecnt_t loop_offset = _num_captured_loops * loop_length.samples();
...
DiskWriter::finish_capture (boost::shared_ptr<ChannelList> c)
{
	...
	if (_loop_location) {
		...
		ci->loop_offset = _num_captured_loops * loop_length.samples();
	} else {
		ci->loop_offset = 0;
	}
